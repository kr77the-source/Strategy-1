...
# =============================================================================
# DATA FETCHING ENGINE (PURE YFINANCE - NO API NEEDED)
# =============================================================================
                ...
                for i in range(slow_len + 1, len(df) - 1):
                    if open_trade is not None:
                        ...
                        if exit_price is not None:
                            ...
                            out_list.append({
                                "Date": open_trade["Date"], "EntryTime": open_trade["EntryTime"], "ExitTime": ts.strftime("%H:%M"),
                                "Symbol": symbol.replace(".NS", ""), "Type": side, "Qty": qty,
                                "EntryPrice": round(entry, 2), "ExitPrice": round(exit_price, 2), "SL": round(open_trade["SL"], 2),
                                "GrossPnL": round(gross, 2), "Charges": chg, "NetPnL": round(gross - chg, 2), "ExitReason": reason
                            })
                            open_trade = None
                            continue
                    ...
                    if trigger and open_trade is None and i + 1 < len(df):
                        ...
                        open_trade = {"Date": df.index[i + 1].strftime("%Y-%m-%d"), "EntryTime": df.index[i + 1].strftime("%H:%M"), "EntryPrice": entry, "SL": sl, "Qty": qty}
...
with tab3:
    ...
    if st.button("▶️ Run 60-Day Backtest", type="primary"):
        with st.spinner("Calculating 60-Day Historical Data via yfinance..."):
            norm_bt, rev_bt = run_60d_backtest(SELECTED_STOCKS, FAST_MA_LEN, SLOW_MA_LEN, ATR_LEN, ATR_MULT, USE_ATR_STOP, TRADE_VALUE, LEVERAGE, TIMEFRAME_MINS, SLIPPAGE_BPS)
            st.session_state.norm_bt, st.session_state.rev_bt = norm_bt, rev_bt

    norm_bt, rev_bt = st.session_state.get("norm_bt"), st.session_state.get("rev_bt")
    if norm_bt is not None and rev_bt is not None:
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("#### Normal Strategy Backtest")
            st.dataframe(norm_bt, use_container_width=True, hide_index=True)
        with c2:
            st.markdown("#### Reversal Strategy Backtest")
